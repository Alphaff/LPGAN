...
def L1_LOSS(batchimg):    #L1_norm是每张图的各个通道的均值 E是每张图每个通道的均值 并没有精确到像素点
...
def L2_LOSS(batchimg):
	L2_norm = tf.norm(batchimg, axis = [1, 2], ord = 2)
	# tf.norm(batchimg, axis = [1, 2], ord = 1) / int(batchimg.shape[1])
	E = tf.reduce_mean(L2_norm)
	return E


def Fro_LOSS(batchimg):
	fro_norm = tf.square(tf.norm(batchimg, axis = [1, 2], ord = 'fro'))
	#fro_norm = tf.norm(batchimg, axis=[1, 2], ord='fro')
...
def LOSS_SSIM(img1,img2,img_f):
	ssim1 = SSIM(img1,img_f)
	ssim2 = SSIM(img2,img_f)
	loss = 1-(ssim1+ssim2)/2
	return loss
